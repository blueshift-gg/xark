...
* `acir-r1cs` — artifact + witness parsing, ACIR → R1CS lowering, opcode
  coverage, opcode rejection. Houses the gadget library (Sha256, Blake2s,
  Blake3, Keccak-f[1600], AES-128, Poseidon2, ECDSA secp256k1/secp256r1,
  variable-index memory).
* `groth16-backend` — Arkworks `ConstraintSynthesizer` wrapper; Groth16
  setup/prove/verify; key + proof serialization (binary + JSON); `.ptau`
  phase-1 parsing and phase-2 contribution; multi-contributor MPC
  ceremony driver; Solana + EVM verifier exports.
* `xark-cli` — the `xark` command-line tool (`inspect`, `setup`,
  `prove`, `verify`, `export`, `ceremony`, `write-vk`).
* `xark-solana-verifier` — on-chain Groth16 verifier program for
  Solana, using `sol_alt_bn128_*` syscalls.
...
* **Solana** on-chain verifier (`xark export solana`, plus the
  `xark-solana-verifier` program).
* **EVM** Solidity verifier (`xark export evm`).
...
See `ROADMAP.md` for what shipped per workstream and `NOIR_VERSION.md`
for the pinned Noir version.
...
Benchmarks (criterion) live under `crates/groth16-backend/benches/`:

```bash
cargo bench -p groth16-backend
# Or save a baseline before a perf PR and compare afterwards:
cargo bench -p groth16-backend -- --save-baseline before
cargo bench -p groth16-backend -- --baseline before
```
...
For Solana on-chain verifier tests, install the Anza CLI and run
`cargo build-sbf` in `crates/xark-solana-verifier/` first; tests gated
behind `--features ark-backend` exercise the SBF binary via Mollusk.
