...
fn compile() -> primitive::PrimitiveProgram {
    let src =
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../examples/blake2s/src/lib.rs");
    let c = xark_test_harness::compile_file(&src, "blake2s", "bn254");
    assert!(
        c.status_success,
        "compiling examples/blake2s failed: {}",
        c.stderr
    );
    c.program()
}
...
fn blake2s_empty_matches_vector() {
    ...
    for (i, d) in digest.iter().enumerate() {
        inputs.insert(id(&format!("d[{i}]")), d.to_string());
    }

    // Honest witness solves and satisfies every constraint.
    let assign = solver::solve_and_check(&program, &inputs).expect("BLAKE2s(\"\") must verify");
...
fn blake2s_abc_matches_vector() {
    ...
    let assign = solver::solve_and_check(&program, &inputs).expect("BLAKE2s(\"abc\") must verify");
    let holes = solver::analyze_underconstrained(&program, &assign);
    assert!(holes.is_empty(), "blake2s under-constrained: {holes:?}");
