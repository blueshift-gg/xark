...
fn compile() -> primitive::PrimitiveProgram {
    let src =
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../examples/blake3/src/lib.rs");
    let c = xark_test_harness::compile_file(&src, "blake3", "bn254");
    assert!(
        c.status_success,
        "compiling examples/blake3 failed: {}",
        c.stderr
    );
    c.program()
}
...
fn blake3_empty_matches_vector() {
    ...
    let assign = solver::solve_and_check(&program, &inputs).expect("BLAKE3(\"\") must verify");
...
fn blake3_abc_matches_vector() {
    ...
    let assign = solver::solve_and_check(&program, &inputs).expect("BLAKE3(\"abc\") must verify");
    let holes = solver::analyze_underconstrained(&program, &assign);
    assert!(holes.is_empty(), "blake3 under-constrained: {holes:?}");
