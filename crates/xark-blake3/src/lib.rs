...
pub fn blake3<const N_BYTES: usize>(msg: [Field; N_BYTES]) -> [[Field; 32]; 8] {
    ...
    const { assert!(N_BYTES <= 1024, "single-chunk BLAKE3 MVP: message must be <= 1024 bytes") };
    ...
    while b < num_blocks {
        ...
        let last_flag = if b + 1usize == num_blocks { 10u64 } else { 0u64 };
