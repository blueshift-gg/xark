...
//!
//! Covers three representative circuits committed under `tests/fixtures/`:
//!
...
//!
//! Run with `cargo bench -p groth16-backend`. Use
//! `cargo bench -p groth16-backend -- --save-baseline before` and
//! `cargo bench -p groth16-backend -- --baseline before` to compare across
//! optimisation PRs.
...
use acir_r1cs::artifact::{parse_artifact_file, NoirArtifact};
...
/// Resolve `tests/fixtures/<name>.{json,gz}` relative to the workspace root.
...
fn fixture_dir() -> PathBuf {
    ...
    loop {
        let candidate = here.join("tests").join("fixtures");
        if candidate.is_dir() {
            return candidate;
        }
        if !here.pop() {
            panic!("could not locate tests/fixtures/ relative to {:?}", std::env::current_dir());
        }
    }
}
...
fn load_fixture(name: &str) -> (NoirArtifact, acir_r1cs::witness::WitnessMap<Fr>, LoweredAcirCircuit) {
    let dir = fixture_dir();
...
fn bench_circuit(c: &mut Criterion, name: &str) {
    ...
    let proof: Proof<Bn254> = {
        ...
        prove(&pk, circuit, &mut rng).expect("prove")
    };
