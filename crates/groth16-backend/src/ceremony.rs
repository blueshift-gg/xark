//! Phase-2 MPC ceremony driver (ROADMAP step **WS-F.2**).
//!
...
/// `delta_g1` from the un-contributed (post-F.1-phase-2) starting state.
pub fn verify_chain(
...
// Silence "unused" warnings for symbols referenced only by the docstring.
#[allow(dead_code)]
fn _doc_anchors(_pk: ProvingKey<Bn254>, _vk: VerifyingKey<Bn254>) {}
    ...
    impl ConstraintSynthesizer<Fr> for Square {
        fn generate_constraints(self, cs: ConstraintSystemRef<Fr>) -> Result<(), SynthesisError> {
            ...
            cs.enforce_constraint(
                ark_relations::r1cs::LinearCombination::from((Fr::one(), x)),
                ark_relations::r1cs::LinearCombination::from((Fr::one(), x)),
                ark_relations::r1cs::LinearCombination::from((Fr::one(), y)),
            )?;
            Ok(())
        }
    }
