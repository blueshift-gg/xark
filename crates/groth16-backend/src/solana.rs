...
//! Ethereum / Solana BN254 precompiles flip the order on the wire, exposing
//! `(c1, c0)`. This module hides that quirk; callers always speak in
//! arkworks-native types.
//!
//! The companion `evm.rs` module emits the same ordering for the Solidity
//! template — see the comment block at the top of that file.
...
// in 32-byte little-endian. The xark-solana-verifier crate uses those by
...
/// `α (G1) || β (G2) || γ (G2) || δ (G2) || ic_count (u32 LE) || ic_count × G1`,
/// every field element in 32-byte little-endian.
pub fn assemble_vk_bytes_le(
    vk: &ark_groth16::VerifyingKey<ark_bn254::Bn254>,
) -> Vec<u8> {
    let mut out = Vec::with_capacity(G1_BYTES + 3 * G2_BYTES + 4 + vk.gamma_abc_g1.len() * G1_BYTES);
