...
//! transcript (ROADMAP step **WS-F.1**).
//!
...
pub fn setup_from_ptau<C: ConstraintSynthesizer<Fr>>(
    ...
    for (row_idx, row) in matrices.c.iter().enumerate() {
        for (coeff, col) in row {
