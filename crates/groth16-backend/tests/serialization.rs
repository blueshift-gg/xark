...
fn fixture_dir() -> PathBuf {
    // crates/groth16-backend/tests/serialization.rs -> crates/groth16-backend/
    // -> crates/ -> workspace root, then `tests/fixtures/groth16/arithmetic_square/`.
...
fn end_to_end_verify_from_fixtures() {
    ...
    assert!(
        ok,
        "committed Groth16 fixtures failed to verify end-to-end against `groth16_backend::verify`"
    );
}
