//! Integration tests for `BlackBoxFuncCall::Keccakf1600` lowering.
//!
//! Exercises the `keccak_basic` Noir fixture end-to-end through the `xark`
//! binary: inspect coverage, setup, prove, verify happy path, plus
//! tampered-output rejection. Mirrors the helpers in `bitwise.rs`
//! intentionally — we deliberately do not share helpers across integration
//! tests (see ROADMAP step WS-D.1).
...
fn workspace_dir() -> PathBuf {
    // crates/xark-cli/tests/keccak.rs -> crates/xark-cli/ -> crates/ -> root.
...
#[test]
fn keccak_basic_verifies() {
    let tmp = tempdir();
    let artifact = fixture_dir().join("keccak_basic.json");
    let witness = fixture_dir().join("keccak_basic.gz");
    let groth_dir = tmp.path().join("groth16");

    // inspect: every opcode (RANGE + Keccakf1600 + AssertZeros) should be
...
#[test]
fn keccak_tampered_first_output_fails() {
    let tmp = tempdir();
    let artifact = fixture_dir().join("keccak_basic.json");
    let witness = fixture_dir().join("keccak_basic.gz");
    let groth_dir = tmp.path().join("groth16");
    ...
    // Flip the lowest bit of the first public output lane.
    ...
    assert!(
        !ok,
        "verify unexpectedly succeeded on tampered first output lane"
    );
    assert!(out.contains("Proof verified: false"), "out={out}");
}

// -----------------------------------------------------------------------------
// Tiny tempdir helper (mirrors the one in bitwise.rs / end_to_end.rs /
...
fn tempdir() -> TempDir {
    ...
    let path = env::temp_dir().join(format!("xark-keccak-test-{pid}-{n}"));
