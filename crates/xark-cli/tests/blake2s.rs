//! Integration tests for `BlackBoxFuncCall::Blake2s` lowering.
//!
//! Exercises the `blake2s_basic` Noir fixture end-to-end through the `xark`
//! binary: inspect coverage, setup, prove, verify happy path, plus
//! tampered-output rejection. Mirrors the helpers in `keccak.rs`
//! intentionally — we deliberately do not share helpers across integration
//! tests (see ROADMAP step WS-D.2).
...
fn workspace_dir() -> PathBuf {
    // crates/xark-cli/tests/blake2s.rs -> crates/xark-cli/ -> crates/ -> root.
...
#[test]
fn blake2s_basic_verifies() {
    let tmp = tempdir();
    let artifact = fixture_dir().join("blake2s_basic.json");
    let witness = fixture_dir().join("blake2s_basic.gz");
    let groth_dir = tmp.path().join("groth16");

    // inspect: every opcode (RANGE + Blake2s + AssertZeros) should be
...
#[test]
fn blake2s_tampered_first_digest_byte_fails() {
    let tmp = tempdir();
    let artifact = fixture_dir().join("blake2s_basic.json");
    let witness = fixture_dir().join("blake2s_basic.gz");
    let groth_dir = tmp.path().join("groth16");
    ...
    // Flip the lowest bit of the first public digest byte.
    ...
    assert!(
        !ok,
        "verify unexpectedly succeeded on tampered first digest byte"
    );
    assert!(out.contains("Proof verified: false"), "out={out}");
}

// -----------------------------------------------------------------------------
// Tiny tempdir helper (mirrors the one in keccak.rs / end_to_end.rs /
// public_inputs_matrix.rs; kept local so test files stay independent).
// -----------------------------------------------------------------------------
...
fn tempdir() -> TempDir {
    ...
    let path = env::temp_dir().join(format!("xark-blake2s-test-{pid}-{n}"));
