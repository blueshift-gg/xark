//! Integration tests for `BlackBoxFuncCall::{AND, XOR}` lowering.
//!
//! Exercises the `bitwise_basic` Noir fixture end-to-end through the
//! `xark` binary: inspect coverage, setup, prove, verify happy path, plus
//! tampered-public-input rejection. Mirrors the helpers in
//! `end_to_end.rs` so this file is self-contained (we deliberately do not
//! share helpers across integration tests — see ROADMAP step WS-B.1).
...
fn workspace_dir() -> PathBuf {
    // crates/xark-cli/tests/bitwise.rs -> crates/xark-cli/ -> crates/ -> root.
...
#[test]
fn bitwise_basic_verifies() {
    let tmp = tempdir();
    let artifact = fixture_dir().join("bitwise_basic.json");
    let witness = fixture_dir().join("bitwise_basic.gz");
    let groth_dir = tmp.path().join("groth16");

    // inspect: every opcode (RANGE + AND + XOR + AssertZeros) should be
...
#[test]
fn bitwise_tampered_xor_output_fails() {
    let tmp = tempdir();
    let artifact = fixture_dir().join("bitwise_basic.json");
    let witness = fixture_dir().join("bitwise_basic.gz");
    let groth_dir = tmp.path().join("groth16");
    ...
    // Flip the lowest bit of the last public input (the XOR result).
    ...
    assert!(!ok, "verify unexpectedly succeeded on tampered xor output");
    assert!(out.contains("Proof verified: false"), "out={out}");
}

// -----------------------------------------------------------------------------
// Tiny tempdir helper (mirrors the one in end_to_end.rs / public_inputs_matrix.rs;
...
fn tempdir() -> TempDir {
    ...
    let path = env::temp_dir().join(format!("xark-bitwise-test-{pid}-{n}"));
