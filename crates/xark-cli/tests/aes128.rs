//! Integration tests for `BlackBoxFuncCall::AES128Encrypt` lowering.
//!
//! Exercises the `aes128_basic` Noir fixture end-to-end through the `xark`
...
fn workspace_dir() -> PathBuf {
    // crates/xark-cli/tests/aes128.rs -> crates/xark-cli/ -> crates/ -> root.
...
#[test]
fn aes128_basic_verifies() {
    let tmp = tempdir();
    let artifact = fixture_dir().join("aes128_basic.json");
    let witness = fixture_dir().join("aes128_basic.gz");
    let groth_dir = tmp.path().join("groth16");

    // inspect: every opcode (RANGE + AES128Encrypt + AssertZeros) should be
...
#[test]
fn aes128_tampered_ciphertext_byte_fails() {
    let tmp = tempdir();
    let artifact = fixture_dir().join("aes128_basic.json");
    let witness = fixture_dir().join("aes128_basic.gz");
    let groth_dir = tmp.path().join("groth16");
    ...
    // Flip the lowest bit of the first public ciphertext byte.
    ...
    assert!(
        !ok,
        "verify unexpectedly succeeded on tampered first ciphertext byte"
    );
    assert!(out.contains("Proof verified: false"), "out={out}");
}

// -----------------------------------------------------------------------------
// Tiny tempdir helper (mirrors the one in blake3.rs / keccak.rs / end_to_end.rs
...
fn tempdir() -> TempDir {
    ...
    let path = env::temp_dir().join(format!("xark-aes128-test-{pid}-{n}"));
