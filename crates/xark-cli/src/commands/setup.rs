...
use groth16_backend::{keys::KeyMetadata, setup, NoirGroth16Circuit};
...
pub fn run(args: SetupArgs) -> Result<()> {
    ...
    if let Some(ptau_path) = args.ptau_file.as_ref() {
        ...
        let ptau =
            groth16_backend::ptau::parse_ptau(&ptau_bytes).with_context(|| "parsing .ptau file")?;
        ...
        let circuit = NoirGroth16Circuit::for_setup(lowered.clone());
        <NoirGroth16Circuit as ark_relations::r1cs::ConstraintSynthesizer<ark_bn254::Fr>>::generate_constraints(
            circuit,
            cs.clone(),
        )
        .map_err(synth_err)?;
