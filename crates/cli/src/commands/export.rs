...
pub fn run(args: ExportArgs) -> Result<()> {
    ...
    // Resolve this before reading keys or creating output. A dirty source build
    // points at its local verifier; clean/released builds remain immutable.
    ...
    if args.verifying_key.is_none() {
        super::ensure_key_matches_current_circuit(&project, &vk_path, None)?;
    }
    ...
    if !verify(&vk, &proof, &public_inputs).map_err(super::synth_err)? {
        anyhow::bail!(
            "refusing to export: {} does not verify against {} and {}. Run `xark prove` for the current setup first",
            proof_path.display(),
            vk_path.display(),
    ...
    write_str(
        &out.join("Cargo.toml"),
        &gen_cargo_toml(&crate_name, &verifier_dep),
    )?;
...
/// built from** (`XARK_GIT_HASH`; `xark --version` prints its short form).
...
/// no user input and no `[patch]`. A crates.io build uses the exact matching
/// release; a dirty source build uses the verifier in that local checkout so
/// unreleased changes can be dogfooded without pretending they exist remotely.
fn verifier_dep() -> Result<String> {
    verifier_dep_for(
        env!("XARK_GIT_HASH"),
...
fn verifier_dep_for(git_hash: &str, version: &str, source_root: &str) -> Result<String> {
    ...
    if git_hash.ends_with("-dirty") {
        ...
        let path = std::path::Path::new(source_root).join("crates/verifier");
        let path = serde_json::to_string(&path.display().to_string())
            .context("encoding local Xark verifier path")?;
...
fn gen_cargo_toml(name: &str, verifier_dep: &str) -> String {
    format!(
...
#[cfg(test)]
mod tests {
    use super::{gen_cargo_toml, verifier_dep, verifier_dep_for};

    #[test]
    fn generated_manifest_is_standalone_and_exactly_sourced() {
        let manifest = gen_cargo_toml("example-verifier", r#""=0.2.2""#);

        // Standalone so cargo doesn't fold it into a neighbouring workspace.
        assert!(manifest.contains("\n[workspace]\n"));
        assert!(!manifest.contains("path ="));
        assert!(!manifest.contains(r#"git = "https://github.com/blueshift-gg/xark" }"#));
    }

    #[test]
    fn verifier_dep_pins_the_build_revision() {
        // A source build pins its exact revision; a released build pins the
        // exact matching verifier release. Neither can drift after generation.
        let dep = verifier_dep();
        if env!("XARK_GIT_HASH").ends_with("-dirty") {
            let expected = std::path::Path::new(env!("XARK_SOURCE_ROOT")).join("crates/verifier");
            assert_eq!(
                dep.unwrap(),
                format!(r#"{{ path = "{}" }}"#, expected.display())
            );
        } else if env!("XARK_GIT_HASH") == "unknown" {
            assert_eq!(dep.unwrap(), format!(r#""={}""#, env!("CARGO_PKG_VERSION")));
        } else {
            let dep = dep.unwrap();
    ...
    #[test]
    fn released_build_pins_the_matching_crate_release() {
        assert_eq!(
            verifier_dep_for("unknown", "0.2.2", "").unwrap(),
            r#""=0.2.2""#
        );
    }

    #[test]
    fn source_build_pins_the_exact_git_revision() {
        assert_eq!(
            verifier_dep_for("abc123", "0.2.2", "").unwrap(),
            r#"{ git = "https://github.com/blueshift-gg/xark", rev = "abc123" }"#
        );
    }

    #[test]
    fn dirty_source_build_uses_its_local_verifier() {
        assert_eq!(
            verifier_dep_for("abc123-dirty", "0.2.2", "/src/xark").unwrap(),
            r#"{ path = "/src/xark/crates/verifier" }"#
        );
    }
}
