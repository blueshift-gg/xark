...
const ECDSA_K1_KAT: &[(&str, &str)] = &[
    ("q.x.limbs[0]", "36791928440626258064573199"),
    ("q.x.limbs[1]", "61528703143672418979960783"),
    ("q.x.limbs[2]", "14747486129163242114008648"),
    ("q.y.limbs[0]", "76282327881385977091575450"),
    ("q.y.limbs[1]", "51294842334813218325050598"),
    ("q.y.limbs[2]", "6439864854718902709630533"),
    ("r.limbs[0]", "12559537206704987108640451"),
    ("r.limbs[1]", "71374797929550670415371493"),
    ("r.limbs[2]", "17349985829470360983052946"),
    ("s.limbs[0]", "68836576935440190365286899"),
    ("s.limbs[1]", "17390598136332323710445584"),
    ("s.limbs[2]", "1526780408919578767236619"),
    ("e.limbs[0]", "59421540702102980326124340"),
    ("e.limbs[1]", "74576772474745118892088547"),
    ("e.limbs[2]", "17542153289124452446895033"),
];
...
const ECDSA_R1_KAT: &[(&str, &str)] = &[
    ...
    ("r.limbs[0]", "49451236000969469476990837"),
    ("r.limbs[1]", "62400969000040298138363671"),
    ("r.limbs[2]", "6553742065852096704011868"),
    ("s.limbs[0]", "40903658636701347218934538"),
    ("s.limbs[1]", "19605469382599579414449386"),
    ("s.limbs[2]", "15625853121683634975831368"),
    ("e.limbs[0]", "59421540702102980326124340"),
    ("e.limbs[1]", "74576772474745118892088547"),
    ("e.limbs[2]", "17542153289124452446895033"),
];
...
fn specs() -> Vec<Spec> {
    ...
    // signature `(r, s)`, and message scalar `e` are all public (15 public inputs).
    v.push(Spec {
        name: "ecdsa_basic",
        n: 15,
        crate_dir: local_dir("ecdsa_basic"),
        private: BTreeMap::new(),
    ...
    v.push(Spec {
        name: "ecdsa_r1_basic",
        n: 15,
        crate_dir: local_dir("ecdsa_r1_basic"),
        private: BTreeMap::new(),
...
fn regen_one(spec: &Spec, work: &Path) {
    ...
    let r1cs =
        xark_ir::json::from_json(&std::fs::read_to_string(gendir.join("r1cs.json")).unwrap())
