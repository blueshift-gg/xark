...
/// secp256k1 ECDSA known-answer vector (86-bit limbs), generated with `k256` and
/// verified off-circuit (`R.x mod n == r`). See the fixture-regen task notes.
const ECDSA_K1_KAT: &[(&str, &str)] = &[
    ...
    ("r0", "12559537206704987108640451"),
    ("r1", "71374797929550670415371493"),
    ("r2", "17349985829470360983052946"),
    ("s0", "68836576935440190365286899"),
    ("s1", "17390598136332323710445584"),
    ("s2", "1526780408919578767236619"),
    ("e0", "59421540702102980326124340"),
    ("e1", "74576772474745118892088547"),
    ("e2", "17542153289124452446895033"),
];
...
const ECDSA_R1_KAT: &[(&str, &str)] = &[
    ...
    ("r0", "49451236000969469476990837"),
    ("r1", "62400969000040298138363671"),
    ("r2", "6553742065852096704011868"),
    ("s0", "40903658636701347218934538"),
    ("s1", "19605469382599579414449386"),
    ("s2", "15625853121683634975831368"),
    ("e0", "59421540702102980326124340"),
    ("e1", "74576772474745118892088547"),
    ("e2", "17542153289124452446895033"),
];
...
const EC_INCOMPLETE_G: &[(&str, &str)] = &[
    ("gx0", "17117865558768631194064792"),
    ("gx1", "12501176021340589225372855"),
    ("gx2", "9198697782662356105779718"),
    ("gy0", "6441780312434748884571320"),
    ("gy1", "57953919405111227542741658"),
    ("gy2", "5457536640262350763842127"),
];
...
const EC_INCOMPLETE_OUT: &[(&str, &str)] = &[
    ("dx0", "57105948487393027623526117"),
    ("dx1", "2088890992725950981549619"),
    ("dx2", "14961784698075395646489684"),
    ("dy0", "46925586441427271765976362"),
    ("dy1", "19820246243853867596485833"),
    ("dy2", "2031033786214458435714136"),
    ("fx0", "57545291876987742944507641"),
    ("fx1", "75066192660561802595210765"),
    ("fx2", "18828234277447069677687620"),
    ("fy0", "2583640362791394057184882"),
    ("fy1", "38197615293098406611150035"),
    ("fy2", "4273588397735691711217203"),
];
...
fn example_dir(gadget: &str) -> PathBuf {
    manifest_dir().join("..").join("..").join("examples").join(gadget)
}
...
fn specs() -> Vec<Spec> {
    ...
    v.push(Spec { name: "arithmetic_square", n: 1, crate_dir: local_dir("arithmetic_square"),
        private: map(&[("x", "3")]), public_in: BTreeMap::new() }); // y = 9 (derived)
    v.push(Spec { name: "arithmetic_public_inputs", n: 1, crate_dir: local_dir("arithmetic_public_inputs"),
        private: map(&[("x", "2"), ("y", "3")]), public_in: BTreeMap::new() }); // out = 11
    v.push(Spec { name: "bitwise_basic", n: 2, crate_dir: local_dir("bitwise_basic"),
        private: map(&[("a", "4042322160"), ("b", "252645135")]), public_in: BTreeMap::new() }); // 0xF0F0F0F0 & 0x0F0F0F0F
    v.push(Spec { name: "range_basic", n: 1, crate_dir: local_dir("range_basic"),
        private: map(&[("x", "200")]), public_in: BTreeMap::new() }); // out = 200
    v.push(Spec { name: "reorder_pi", n: 2, crate_dir: local_dir("reorder_pi"),
        private: map(&[("b", "4")]), public_in: map(&[("a", "7"), ("c", "9")]) }); // b*b = 16 = 7+9
    v.push(Spec { name: "mixed_pi", n: 2, crate_dir: local_dir("mixed_pi"),
        private: map(&[("x", "2")]), public_in: map(&[("y", "3")]) }); // ret = 2*3+2 = 8
    v.push(Spec { name: "return_values_only", n: 1, crate_dir: local_dir("return_values_only"),
        private: map(&[("x", "4")]), public_in: BTreeMap::new() }); // ret = 16
    v.push(Spec { name: "large_pi", n: 16, crate_dir: local_dir("large_pi"),
        ...
        public_in: {
            ...
            for i in 1..15 { m.insert(format!("x{i}"), "1".to_string()); }
            m
        } });
    v.push(Spec { name: "memory_const", n: 1, crate_dir: local_dir("memory_const"),
        private: map(&[("x", "5")]), public_in: BTreeMap::new() }); // y = 6*5 = 30
    v.push(Spec { name: "memory_var", n: 1, crate_dir: local_dir("memory_var"),
        private: map(&[("arr0", "10"), ("arr1", "20"), ("arr2", "30"), ("arr3", "40"),
                       ("sel0", "0"), ("sel1", "0"), ("sel2", "1"), ("sel3", "0")]),
        public_in: BTreeMap::new() }); // y = arr2 = 30
    v.push(Spec { name: "multi_function", n: 1, crate_dir: local_dir("multi_function"),
        private: map(&[("x", "6")]), public_in: BTreeMap::new() }); // y = 36
    v.push(Spec { name: "nested_calls", n: 1, crate_dir: local_dir("nested_calls"),
        private: map(&[("x", "6")]), public_in: BTreeMap::new() }); // y = 6*6 + 1 = 37

    // ---- Gadget fixtures (real gadgets, from examples/) ----
    // Hash gadgets: arbitrary private message words; public digest is derived.
    v.push(Spec { name: "sha256_basic", n: 8, crate_dir: example_dir("sha256"),
        private: map(&[("m[0]", "1"), ("m[1]", "2")]), public_in: BTreeMap::new() });
    v.push(Spec { name: "aes128_basic", n: 16, crate_dir: example_dir("aes"),
        private: {
            let mut m = arr("pt", 16, 0);
            m.extend(arr("key", 16, 16));
            m
        }, public_in: BTreeMap::new() });
    v.push(Spec { name: "keccak_basic", n: 4, crate_dir: example_dir("keccak"),
        private: arr("words", 17, 1), public_in: BTreeMap::new() });
    v.push(Spec { name: "blake2s_basic", n: 9, crate_dir: example_dir("blake2s"),
        private: arr("m", 16, 1), public_in: map(&[("len", "64")]) });
    v.push(Spec { name: "blake3_basic", n: 9, crate_dir: example_dir("blake3"),
        private: arr("m", 16, 1), public_in: map(&[("len", "64")]) });
    v.push(Spec { name: "poseidon_basic", n: 3, crate_dir: example_dir("poseidon2"),
        private: map(&[("in0", "1"), ("in1", "2"), ("in2", "3")]), public_in: BTreeMap::new() });

    // secp256k1 EC 2G/3G: private generator limbs; public 2G/3G outputs derived.
    v.push(Spec { name: "curve_basic", n: 12, crate_dir: example_dir("ec_incomplete"),
        private: map(EC_INCOMPLETE_G), public_in: map(EC_INCOMPLETE_OUT) });

    // ECDSA: all-private inputs (0 public).
    v.push(Spec { name: "ecdsa_basic", n: 0, crate_dir: local_dir("ecdsa_basic"),
        private: map(ECDSA_K1_KAT), public_in: BTreeMap::new() });
    v.push(Spec { name: "ecdsa_r1_basic", n: 0, crate_dir: local_dir("ecdsa_r1_basic"),
        private: map(ECDSA_R1_KAT), public_in: BTreeMap::new() });
...
fn xark_toolchain_bin() -> PathBuf {
    ...
    assert!(status.success(), "failed to build the xark toolchain binary");
...
fn setup(r1cs: &R1csProgram) -> Groth16Keys {
    ...
    loop {
        ...
        match setup_from_ptau(XarkCircuit::for_setup(r1cs.clone()), &ptau, &seed) {
            Ok(keys) => return keys,
            Err(Phase2Error::PtauTooSmall { required_domain_size, .. }) => {
...
fn eval_lc(lc: &IrLc, assign: &BTreeMap<VarId, Fr>) -> Fr {
    ...
    for t in &lc.terms {
        acc += fr_from_decimal(&t.coeff.decimal) * assign.get(&t.var).copied().unwrap_or_else(Fr::zero);
    }
    acc
}
...
fn derive_public(r1cs: &R1csProgram, assign: &BTreeMap<VarId, Fr>, p: VarId, name: &str) -> Fr {
    for con in &r1cs.constraints {
        ...
        assert!(!denom.is_zero(), "public output `{name}` is unconstrained in constraint {}", con.id);
...
fn regen_one(spec: &Spec, work: &Path) {
    ...
    let r1cs = xark_ir::json::from_json(
        &std::fs::read_to_string(gendir.join("r1cs.json")).unwrap(),
    )
    .expect("parse r1cs.json");
    let prim = primitive::from_json(&std::fs::read_to_string(gendir.join("circuit.json")).unwrap())
    ...
    for var in &prim.vars {
        match var.role {
            ...
            primitive::VarRole::PublicInput => {
                let val = spec.public_in.get(&var.name).cloned().unwrap_or_else(|| "0".to_string());
    ...
    let dir = manifest_dir().join("fixtures").join("groth16").join(spec.name);
