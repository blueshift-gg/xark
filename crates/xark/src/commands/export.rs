...
pub struct ExportArgs {
    ...
    /// The `xark-verifier` dependency spec written into the generated crate's
    /// `Cargo.toml`, verbatim after `xark-verifier = `. Examples:
    /// `"0.0.1"`, `{ path = "../xark/crates/verifier" }`,
    /// `{ git = "https://github.com/blueshift-gg/xark", branch = "master" }`.
    /// Defaults to a git dependency on this repo, so the crate builds on a
    /// fresh clone until `xark-verifier` is published to crates.io.
    #[arg(long)]
    pub verifier_dep: Option<String>,
...
pub fn run(args: ExportArgs) -> Result<()> {
    ...
    write_str(
        &out.join("Cargo.toml"),
        &gen_cargo_toml(&crate_name, args.verifier_dep.as_deref()),
    )?;
...
/// The default `xark-verifier` dependency: a git source on this repo, so the
/// generated crate compiles on a fresh clone without a sibling checkout or a
/// `[patch]`. Overridable with `xark export --verifier-dep <SPEC>`; switch to a
/// plain version once `xark-verifier` is published to crates.io.
const DEFAULT_VERIFIER_DEP: &str = r#"{ git = "https://github.com/blueshift-gg/xark" }"#;

fn gen_cargo_toml(name: &str, verifier_dep: Option<&str>) -> String {
    let verifier_dep = verifier_dep.unwrap_or(DEFAULT_VERIFIER_DEP);
    format!(
...
[dependencies]
# The verifier core. Override the source with `xark export --verifier-dep`.
...
mod tests {
    ...
    #[test]
    fn generated_manifest_is_standalone_and_buildable_by_default() {
        let manifest = gen_cargo_toml("example-verifier", None);

        assert!(manifest.contains("\n[workspace]\n"));
        assert!(manifest
            .contains(r#"xark-verifier = { git = "https://github.com/blueshift-gg/xark" }"#));
    }

    #[test]
    fn generated_manifest_uses_custom_verifier_dependency() {
        let manifest = gen_cargo_toml(
            "example-verifier",
            Some(r#"{ path = "../../crates/verifier" }"#),
        );

        assert!(manifest.contains(r#"xark-verifier = { path = "../../crates/verifier" }"#));
    }
}
