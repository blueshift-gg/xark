...
use rustc_middle::mir::{
    Body, Operand, Place, Rvalue, StatementKind, TerminatorKind,
};
...
pub fn validate<'tcx>(tcx: TyCtxt<'tcx>, body: &Body<'tcx>) -> CompileResult<()> {
    for bb in body.basic_blocks.indices() {
        ...
        check_terminator(tcx, &terminator.kind).map_err(|e| e.or_span(terminator.source_info.span))?;
    }
    Ok(())
}
...
fn check_rvalue(rvalue: &Rvalue<'_>) -> CompileResult<()> {
    match rvalue {
        ...
        Rvalue::Cast(..) => Err(unsupported_rvalue("casts (only compile-time integer casts)")),
