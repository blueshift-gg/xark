...
pub use r1cs::{
    DebugInfo, FieldSpec, R1csConstraint, R1csProgram, Variable, Visibility,
};
...
mod tests {
    ...
    fn difference_of_squares_is_one_constraint() {
        ...
        let lhs = LinearCombination::var(x).add(LinearCombination::var(y));
        let rhs = LinearCombination::var(x).sub(LinearCombination::var(y));

        let program = R1csProgram {
            field: FieldSpec::unknown(),
            variables: vec![
                Variable { id: x, name: "x".into(), visibility: Visibility::Private },
                Variable { id: y, name: "y".into(), visibility: Visibility::Private },
                Variable { id: z, name: "z".into(), visibility: Visibility::Public },
            ],
    ...
    fn mulmod_divmod_hint_computes_big_division() {
        ...
        for (i, n) in ["a0", "a1", "a2", "a3", "b0", "b1", "b2", "b3"].iter().enumerate() {
            vars.push(Var { id: i as u32, name: n.to_string(), role: VarRole::PrivateInput });
        }
        for (i, n) in ["q0", "q1", "q2", "q3", "r0", "r1", "r2", "r3"].iter().enumerate() {
            vars.push(Var { id: 8 + i as u32, name: n.to_string(), role: VarRole::Derived });
        }
    ...
    fn analyzer_passes_sound_and_flags_underconstraints() {
        ...
        let mul_expr = |l: u32, r: u32, out: u32| Expression {
            mul_terms: vec![MulTerm { coeff: FieldConst::one(), left: l, right: r }],
            linear_terms: vec![LinearTerm { coeff: FieldConst::from_i64(-1), var: out }],
        ...
        let sound = PrimitiveProgram {
            ...
            vars: vec![
                ...
                Var { id: 2, name: "t".into(), role: VarRole::Derived },
            ],
        ...
        dangling.vars.push(Var { id: 3, name: "w".into(), role: VarRole::Derived });
        ...
        assert!(flags.iter().any(|f| f.name == "w"), "dangling advice must be flagged: {flags:?}");
        ...
        let two_valued = PrimitiveProgram {
            field: FieldSpec::bn254(),
            vars: vec![Var { id: 0, name: "bit".into(), role: VarRole::Derived }],
            constraints: vec![Expression {
                mul_terms: vec![MulTerm { coeff: FieldConst::one(), left: 0, right: 0 }],
                linear_terms: vec![LinearTerm { coeff: FieldConst::from_i64(-1), var: 0 }],
                constant: FieldConst::zero(),
                note: None,
            }],
    ...
    fn scale_folds_and_big_constants_are_exact() {
        // 3 * (x + 2) = 3x + 6
        let x = 0u32;
        let lc = LinearCombination::var(x)
            .add(LinearCombination::constant("2"))
    ...
    fn primitive_ir_bit_decompose_solves_and_checks() {
        ...
        let mut vars = vec![Var { id: x, name: "x".into(), role: VarRole::PrivateInput }];
        for (i, id) in bit_ids.iter().enumerate() {
            vars.push(Var { id: *id, name: format!("w{i}"), role: VarRole::Derived });
        }
        ...
        for id in &bit_ids {
            constraints.push(Expression {
                mul_terms: vec![MulTerm { coeff: FieldConst::one(), left: *id, right: *id }],
                linear_terms: vec![LinearTerm { coeff: FieldConst::from_i64(-1), var: *id }],
                constant: FieldConst::zero(),
                note: Some(format!("w{id} boolean")),
            });
        }
        // Recomposition: Σ 2^i wi - x = 0.
        let mut linear_terms = vec![LinearTerm { coeff: FieldConst::from_i64(-1), var: x }];
        for (i, id) in bit_ids.iter().enumerate() {
    ...
    fn lc_simplification_combines_and_drops() {
        ...
        let x = 5u32;
        let lc = LinearCombination::var(x)
            .add(LinearCombination::var(x))
            .sub(LinearCombination {
                constant: FieldConst::zero(),
                terms: vec![Term { coeff: FieldConst::from_i64(2), var: x }],
            });
