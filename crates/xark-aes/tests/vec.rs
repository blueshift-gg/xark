...
fn id_of(p: &PrimitiveProgram, name: &str) -> u32 {
    p.vars.iter().find(|v| v.name == name).map(|v| v.id).unwrap()
}
...
fn gf_mul_matches_known_products() {
    ...
    for &(a, b, c) in &[(0x53u32, 0xCAu32, 0x01u32), (0x57, 0x83, 0xC1), (0x02, 0x87, 0x15)] {
        ...
        assert!(solver::solve_and_check(&p, &inputs).is_err(), "wrong product accepted");
    }
}
...
fn sbox_matches_fips_table() {
    ...
    assert!(solver::solve_and_check(&p, &inputs).is_err(), "wrong sbox accepted");
}
...
fn key_schedule_matches_fips() {
    ...
    let mut inputs = BTreeMap::new();
    for i in 0..16 { inputs.insert(id_of(&p, &format!("k{i}")), key[i].to_string()); }
    // FIPS-197 A.1: word4=0xd6aa74fd; word40 byte0=0x13; word43 byte3=0xc5.
    for (n, v) in [("e16", 0xd6u32), ("e17", 0xaa), ("e18", 0x74), ("e19", 0xfd),
                   ("e160", 0x13), ("e175", 0xc5)] {
        inputs.insert(id_of(&p, n), v.to_string());
    }
    let a = solver::solve_and_check(&p, &inputs);
    if let Err(e) = &a { eprintln!("KS solve err at {e:?}"); }
...
fn aes128_matches_fips_kat() {
    ...
    let assign = solver::solve_and_check(&p, &inputs)
        .expect("AES-128 FIPS KAT must verify");
