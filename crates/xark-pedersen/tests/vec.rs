...
fn load_program() -> primitive::PrimitiveProgram {
    let src = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../examples/pedersen/src/lib.rs");
    let c = xark_test_harness::compile_file(&src, "pedersen", "bn254");
    assert!(c.status_success, "compiling examples/pedersen failed: {}", c.stderr);
    c.program()
}
...
fn pedersen_matches_reference_vector() {
    ...
    let assign =
        solver::solve_and_check(&program, &inputs).expect("Pedersen circuit must verify on the reference vector");
