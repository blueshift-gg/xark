...
impl Circuit {
    ...
    /// by `#[circuit]` (e.g. `CubeInputs { secret: 2, result: 4 }`). Each value
    /// is matched to a declared `Private`/`Public` input by name and reduced into
    /// the field (negatives wrap mod p).
    ///
    /// The witness is solved and checked; an unsatisfiable one returns `false`.
    /// Otherwise a dev-mode Groth16 (seed 1) setup + prove + verify runs and its
    /// result is returned — a valid witness yields `true`, an invalid one `false`.
    ///
    /// Panics on an input whose name doesn't match a declared input, or a count
    /// mismatch.
    pub fn prove<I: ProveInputs>(&self, inputs: I) -> bool {
        // Declared inputs (Private/Public), indexed by name.
