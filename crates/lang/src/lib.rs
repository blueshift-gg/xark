...
//! programs in the `xark` Rust subset. A circuit is an ordinary stable Rust
//! function over [`Field`] values marked with [`circuit`]; the toolchain compiles
//! it end to end (rustc-MIR → xark-IR → R1CS → Groth16) with no per-gadget
//! backend support — gadgets are
...
/// Host-side implementation details used by code generated by `xark-macros`.
///
/// This is public only because proc-macro expansion happens in the downstream
/// crate. It is deliberately not part of the documented authoring API: circuit
/// crates depend on `xark` alone, while this module owns the prover plumbing
/// needed by generated native validators and typed inputs.
#[cfg(not(xark))]
#[doc(hidden)]
pub mod __private {
    pub use xark_prover::{
        NativeInput, ProveError, ProveInputs, circuit, circuit_at, limb_leaves, limb_leaves_decimal,
    };
}
