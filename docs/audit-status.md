...
### 5. Trusted-setup ceremony (`groth16-backend/src/setup_phase2.rs`, `ceremony.rs`, `ptau.rs`)
...
### 6. Serialisation boundaries (`groth16-backend/src/{serialization,solana,evm}.rs`)

* Binary VK/proof format — pinned by
  [`serialization.md`](serialization.md) and snapshot-tested.
* Solana export — Ethereum-compatible big-endian uncompressed
  encoding with a *flipped* `Fq2` `(c1, c0)` order. Easy to get
  wrong; the unit test against
  `ark_bn254::Fq2`'s canonical form catches it.
* EVM verifier — Solidity template with VK constants. A subtle bug
  here breaks on-chain verification silently.
