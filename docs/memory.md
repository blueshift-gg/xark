...
> **Status: implemented.** Both phases below have shipped — constant-index
> *and* variable-index memory, including variable-index **writes**. The code
> lives in `crates/acir-r1cs/src/opcodes/memory.rs`; this file is kept as the
> design note / soundness argument it grew from. Read it for the *why*; read
> the source for the *what*. Sections written in the future tense ("when
> starting C.5…", "what to test…") are historical.

This document is the **design note** for memory-opcode lowering. The
implementation is split into two parts:

* constant-index memory (the easy 80%).
* variable-index memory via a selector argument.

For the constant-index part, follow the "Constant-index lowering" section
below. For the variable-index part, follow the "Variable-index plan" section.
...
## Decision: constant-index first, variable-index later
...
1. **Happy path.** A Noir program with `let a = [1, 2, 3]; assert(a[0] ==
 1)` proves and verifies. Add as `crates/tests/circuits/array_const_index`.
