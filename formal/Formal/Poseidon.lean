...
`poseidon.rs::sbox` emits three constraints for one cell:
...
by `poseidon.rs::sbox` force the output to be `x⁵`. -/
...
/-- The `x⁵` S-box as a Lean function (matches `single_box` in `poseidon.rs`). -/
...
unchanged — `poseidon.rs` does exactly this in its `rf_half..p_end` loop.
...
column of `rc_table[r]` for internal rounds, per `poseidon.rs`). -/
...
end-of-soundness statement for `poseidon.rs` — combined with `sbox_sound`,
