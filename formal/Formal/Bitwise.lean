...
/-- **AND-bit soundness.** `bitwise.rs::and` emits `aᵢ * bᵢ = outᵢ`. Given the
...
/-- **XOR-bit soundness.** `bitwise.rs::xor` emits `(2·a) · b = a + b − out`
...
/-- **NOT-bit soundness.** `bitwise.rs::not` represents the complement as the
